...
def get_path_for_config(config_name):
    """
    :param config_name: config name, as taken from the configuration JSON
    :return: path to the folder with the results for this config
    """
    return "../../Results/{}".format(config_name)


def make_gif(config_name):
    """
    saves a gif movie from all the png files found in the folder represented by config_name argument
    each frame in the gif has a duration of 0.1 seconds
    :param config_name: the configuration name as taken from the configuration JSON
    """
    path = get_path_for_config(config_name)
...
def calc_icp(velodyne_frame, velodyne_frame_prev, frame_idx, config_name):
    ...
    :param config_name: the name of the configuration as taken from the configuration JSON
                        ...
                        if the configuration name argument is None, the image is simply shown as prompt
    ...
    while delta_err > 1e-16 and iter < max_iterations:
        ...
        P_center = find_center_of_mass(P_matching).reshape(1, 3)
        P_centered = P_matching - P_center

        Q_center = find_center_of_mass(Q_matching).reshape(1, 3)
    ...
    _, velodyne_raw_bev_Q = process_frame(np.concatenate([Q, np.zeros((Q.shape[0], 1))], axis=1) - offset, 0, frame_idx)
    _, velodyne_frame_transformed_copy_bev = process_frame(velodyne_frame_copy - offset, 0, frame_idx - 1)
    _, velodyne_frame_prev_copy_bev = process_frame(velodyne_frame_prev_copy - offset, 0, frame_idx)
    ...
    if config_name:
        ...
        im2.save("{}/{:04d}.png".format(get_path_for_config(config_name + "_ICP_Matching"), frame_idx), format='PNG')
    else:
        plt.show()
...
def transform_velodyne(velodyne_frame, curr_e, curr_n, curr_u, curr_yaw, velo_to_imu_R, velo_to_imu_t):
    ...
    t = np.array([curr_e, curr_n, curr_u, 0]).reshape(4, 1)
...
def filter_velodyne_data(velodyne_raw, min_height=ABOVE_GROUND_THRESHOLD):
    """
    ...
    for point in velodyne_raw:
        ...
        if height_val <= (min_height - VELODYNE_HEIGHT_METERS):
            continue
...
def update_occupancy_map(velodyne_frame_filtered, occupancy_map, car_pos_x, car_pos_y, yaw, l_occ, l_free, is_async=False):  # TODO(ofekp): True
    ...
    velodyne_frame_filtered_copy = np.concatenate((velodyne_frame_filtered_copy, np.arctan2(velodyne_frame_filtered_copy[:, 1], velodyne_frame_filtered_copy[:, 0]).reshape(velodyne_frame_filtered_copy.shape[0], 1)), axis=1)
    ...
    u_range = range(int(car_pos_u - max_range_uv), int(car_pos_u + max_range_uv))
    v_range = range(int(car_pos_v - max_range_uv), int(car_pos_v + max_range_uv))
...
def render_frame(img, velodyne_raw_bev, occupancy, frame_idx, config_name):
    """
    ...
    :param frame_idx: the current frame index
    :param config_name: the name of the config as taken from the configuration JSON
    """
    ...
    im = Image.open(ram)
    im2 = im.convert('RGB').convert('P', palette=Image.ADAPTIVE)
    im2.save("{}/{:04d}.png".format(get_path_for_config(config_name), frame_idx), format='PNG')
...
def process_frame(velodyne_frame, yaw, frame_idx):
    ...
    for point in velodyne_frame_copy:
        ...
        velodyne_raw_bev[map_size - int(point[1]) - 1, int(point[0])] = max(velodyne_raw_bev[map_size - int(point[1]) - 1, int(point[0])], height_val)
...
def main():
    ...
    rpy_enu[:, :] = oxts[:, [4, 5, 6]]
    rpy_in_ned = rpy_enu.copy()   # this line caused me a lot of time before the copy(), should have copied the vector from the start :(
    ...
    ax1.legend(["Raw east/north samples", "east/north with noise (sigma=0.5m)"], prop={"size": 20}, loc="best")
    ...
    # sanity for process_image and Velodyne data
    curr_frame = 15
    velodyne_frame = velodyne["frames_raw"][curr_frame].copy()
    _, velodyne_frame_transformed_bev = process_frame(velodyne_frame, 0, curr_frame)
    map_size = int((2 * MAX_RANGE_RADIUS_METERS) / MAP_RESOLUTION)
    ...
    ax1.set_xlabel('X [meters]', fontsize=20)
    ax1.set_ylabel('Y [meters]', fontsize=20)
    ax1.set_title('A single Velodyne frame [{}] direcly from the dataset'.format(curr_frame))
    ...
    velodyne_frame_transformed = transform_velodyne(velodyne_frame, enu[curr_frame, 0],
                                                    enu[curr_frame, 1], enu[curr_frame, 2],
                                                    yaw[curr_frame], velo_to_imu_R, velo_to_imu_t)
    velodyne_frame_prev_transformed = transform_velodyne(velodyne_frame_prev, enu[prev_frame, 0], enu[prev_frame, 1],
                                                         enu[prev_frame, 2], yaw[prev_frame],
                                                         velo_to_imu_R, velo_to_imu_t)
    _, velodyne_frame_transformed_bev = process_frame(
        np.concatenate([velodyne_frame_transformed, velodyne_frame_prev_transformed], axis=0), 0, curr_frame)
    fig, (ax1) = plt.subplots(1, 1)
    ...
    velodyne_frame_enu = transform_velodyne(velodyne_frame_filtered, enu[curr_frame, 0] + delta_e,
                                        enu[curr_frame, 1] + delta_n, enu[curr_frame, 2],
                                        yaw[curr_frame] + delta_yaw, velo_to_imu_R, velo_to_imu_t)
    velodyne_frame_prev_filtered = velodyne_frame_filtered.copy()  # same exact frame for sanity
    velodyne_frame_prev_enu = transform_velodyne(velodyne_frame_prev_filtered, enu[curr_frame, 0],
                                        enu[curr_frame, 1], enu[curr_frame, 2],
                                        yaw[curr_frame], velo_to_imu_R, velo_to_imu_t)
    ...
    # velodyne_frame_enu = (np.dot(rotation, velodyne_frame_enu.T) + translation).T
    ...
    # rotation = np.identity(4)
    # rotation[0:2, 0:2] = np.array([[np.cos(yaw_icp_diff), np.sin(yaw_icp_diff)], [-np.sin(yaw_icp_diff), np.cos(yaw_icp_diff)]]).reshape(2, 2)
    # translation = np.array([e_icp_diff, n_icp_diff, 0, 0]).reshape(4, 1)
    # velodyne_frame_corrected = (np.dot(rotation, velodyne_frame_enu.T) - translation).T
    # _, velodyne_frame_transformed_bev = process_frame(np.concatenate([velodyne_frame_corrected, velodyne_frame_prev_enu], axis=0), 0, curr_frame)
    # fig, (ax1) = plt.subplots(1, 1)
    #
    # map_size = int((2 * MAX_RANGE_RADIUS_METERS) / MAP_RESOLUTION)
    # ax1.set_xticks([0, (map_size // 2), map_size - 2])
    ...
    # ax1.set_xlabel('X [meters]', fontsize=20)
    # ax1.set_ylabel('Y [meters]', fontsize=20)
    # ax1.set_title('Moving a transposed frame according to the ICP result'.format(curr_frame, prev_frame))
    # ax1.imshow(velodyne_frame_transformed_bev, vmin=0.0, vmax=1.0)
    #
    # plt.show()
    ...
    # assert abs(e_icp_diff - delta_x) < 0.01
    # assert abs(n_icp_diff - delta_y) < 0.01
    # assert abs(yaw_icp_diff - (-delta_rad)) < 0.01
    ...
    for config in configs:
        ...
        config_results_path = get_path_for_config(config_name)
        if not os.path.isdir(config_results_path):
            os.mkdir(config_results_path)
        print("Now processing config [{}]...".format(config_name))
        for frame_idx in range(frame_count):
            ...
            if apply_icp and frame_idx > 0:
                ...
                im = Image.open(ram)
                im2 = im.convert('RGB').convert('P', palette=Image.ADAPTIVE)
                im2.save("{}/real_noisy_and_corrected_data.png".format(get_path_for_config(config_name), frame_idx), format='PNG')
            ...
            velodyne_frame_filtered_for_occupancy_map = filter_velodyne_data(velodyne_frame.copy())
            update_occupancy_map(velodyne_frame_filtered_for_occupancy_map, occupancy_map, car_pos_x, car_pos_y, float(yaw_from_config[frame_idx]), l_occ, l_free)
            ...
            if is_animation or frame_idx == frame_count - 1:
                ...
                render_frame(img, velodyne_raw_bev, occupancy_map_render, frame_idx, config_name)
            print("Frame index [{}] done in [{}] seconds".format(frame_idx, int(time.time() - ts)))
        if is_animation:
            make_gif(config_name)
